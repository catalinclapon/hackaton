...
public class MedicalCaseService {
	...
	public MedicalCaseDTO save(MedicalCaseDTO medicalCaseDTO) {
		...
		if (medicalCaseDTO.getId() != null) {
			medicalCaseRepository.save(Optional.of(medicalCaseDTO).map(MedicalCaseDTO::build).map(mc -> {
		...
		MedicalCase medicalCase = medicalCaseRepository.save(Optional.of(medicalCaseDTO).map(MedicalCaseDTO::build)
				.map(mc -> mc.id(null).status(finalMedicalCaseStatus)).get());

		medicalCaseDTO.setId(medicalCase.getId());

		medicalCaseDTO.getFields().stream().map(MedicalCaseFieldDTO::build)
				.map(mc -> mc.id(null).medicalCase(medicalCase)).forEach(medicalCaseFieldRepository::saveAndFlush);
	...
	private List<Map<String, String>> findCasesByGroup(List<MedicalCase> cases, List<Long> fields,
			List<UserGroup> currentUserGroupList) {
		List<Map<String, String>> result = new ArrayList<>();
	...
	public List<Map<String, String>> findCases(String registryUuid, List<Long> fields) throws Exception {
		...
		log.debug("LISTA A FOST CREATA!");
		log.debug("Lungimea listei este: {}", cases.size());
		log.debug("Statusul este: {}", cases.get(0).getStatus());
		if (CollectionUtils.isEmpty(userGroupRepository.findByUserIsCurrentUser())) {
			...
			} else if (authority.getName().equals(AuthoritiesConstants.DOCTOR)
					|| authority.getName().equals(AuthoritiesConstants.PROVIDER)) {
	...
	public MedicalCaseDTO update(MedicalCaseDTO medicalCaseDTO) {
		...
		// medicalCaseRepository.findByCNP(medicalCaseDTO.getFields().get(2).getValue());
		String cnp = medicalCaseDTO.getFields().stream() // MedicalCaseFieldDTO
															// stream
				.filter(mcf -> mcf.getField().getName().equals("CNP")).map(mcf -> mcf.getValue())
		...
		for (MedicalCaseField medicalCaseField : managedFields) {
			medicalCaseField.value(dtoFields.get(i).getValue()).medicalCase(medicalCase);
			i++;
		}
		medicalCase.status("LATEST").setPatientCnp(patientRepository.findByUserIsCurrentUser().get(0).getCnp());

		// when transaction is completed, all the changes made to the managed
		// medicalCase should be flushed
		return MedicalCaseDTO.build(medicalCase);
	}
	...
	public MedicalCaseDTO updateStatus(MedicalCaseDTO medicalCaseDTO) {
		log.debug("Regquest to update the status for a medical case: {}", medicalCaseDTO);
		MedicalCase medicalCase = medicalCaseDTO.build(medicalCaseDTO);

		medicalCaseRepository.setStatusForMedicalCase(medicalCase.getStatus(), medicalCase.getId());
